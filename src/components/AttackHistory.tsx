...
const AttackHistory = ({ threats, blockedIps = [] }: AttackHistoryProps) => {
            ...
            <div className="absolute right-0 top-full mt-1 z-50 w-48 rounded-lg border border-border bg-popover shadow-xl">
              <div className="p-1">
                {DB_OPTIONS.map((db) => (
                  <button
                    key={db.value}
                    onClick={() => { setSelectedDb(db.value); setDropdownOpen(false); }}
                    className={`w-full flex items-center gap-2 px-3 py-2 text-xs rounded-md transition-colors ${
                      selectedDb === db.value ? "bg-primary/10 text-primary" : "text-foreground hover:bg-muted"
                    }`}
                  >
                    <span>{db.icon}</span>
                    <span>{db.label}</span>
                    {selectedDb === db.value && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-primary" />}
                  </button>
                ))}
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 flex items-center gap-2">
                      ...
                      {isBlocked ? (
                        <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded border bg-destructive/20 text-destructive border-destructive/30">
                          🚫 BLOCKED
                        </span>
                      ) : (
